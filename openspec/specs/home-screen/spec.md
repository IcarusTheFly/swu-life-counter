...
#### Scenario: Record without draws still shows all three numbers
- **GIVEN** a deck with 8 wins, 3 losses, and no draws
- **WHEN** the Home TOP DECKS list renders that deck
- **THEN** the card shows the record as `8-3-0` (the `0` draws is shown, never `8-3`, never the literal "W-L-D" letters)

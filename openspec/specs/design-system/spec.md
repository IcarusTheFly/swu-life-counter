...
The `design-system` capability defines the app's metallic + gold design system: a single source-of-truth tokens module (no React, no platform imports) and a shared `components/ui` kit built on top of it. It is the common visual foundation — metallic gradients (silver / gold / steel / crimson), a gold secondary accent, contrast-tiered text, surfaces, borders, type scale, spacing, radii, and elevation — that every screen references instead of hard-coding its own metallic styling.
